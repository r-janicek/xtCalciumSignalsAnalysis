function plotProfileAnalysis2(x_t,selectedROIs,TPP_delays,durTPP,pxSz_x,pairedAnalysis,analysisType)
...
for i = 1:height(selectedROIs)
    ...
    for k=1:height(selectedROIs.AnalysisResult{i,1})
        ...
        if strcmp(pairedAnalysis,'Yes') 
            ...
            x1 = selectedROIs.allPeaksData{i,1}.peakPosFit(2*k-1);
            x2 = selectedROIs.allPeaksData{i,1}.peakPosFit(2*k);
            [~,indx1] = min(abs(t_fit-x1));
            [~,indx2] = min(abs(t_fit-x2));
            A1 = selectedROIs.allPeaksData{i,1}.AmplitudeFit(2*k-1) + baselineFit(indx1);
            A2 = selectedROIs.allPeaksData{i,1}.AmplitudeFit(2*k) + baselineFit(indx2);
            ym = max(A1,A2)+0.1;
