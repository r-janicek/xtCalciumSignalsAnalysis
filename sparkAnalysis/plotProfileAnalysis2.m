...
for i = 1:height(selectedROIs)
    ...
    if mod(i,3)==1
        a = a+1;
        hf = figure( ...
            'Name',sprintf('repetitive sparks analysis whole profile fit #%d',a), ...
    ...
    for k=1:height(selectedROIs.AnalysisResult{i,1})
        ...
        str = sprintf('A2/A1 = %.2f \n %.2f (ms)', ...
            selectedROIs.AnalysisResult{i,1}.AmplRatioFit(k), ...
            selectedROIs.AnalysisResult{i,1}.peakPosDiffFit(k));
        text(x1+(x2-x1)/2, ym + 0.2, str, 'Parent',h_axes(aa(i),1), ...
    ...
    for j=1:size(selectedROIs.eventsPeaks{i,1},1)
        ...
        line(t_fit, y_fit, 'Parent',h_axes(aa(i),1), ...
            'Color','b', 'LineWidth',1, 'LineStyle','-')
        line([pos pos], [baseline Ampl+baseline], ...
            'Parent',h_axes(aa(i),1), 'Color','m', ...
            'LineWidth',1,'LineStyle','-')
        
        str = sprintf('A = %.2f',Ampl);
        text(t0-15, Ampl/2+baseline, str, 'Parent',h_axes(aa(i),1), ...
            'HorizontalAlignment','center', 'FontSize',8,...
