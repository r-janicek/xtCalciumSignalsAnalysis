...
if numel(profileAnalysis.statEventsSpRec) ~= numel(profileAnalysis.h_PeaksCirc)   
...
h_rect_pos = getPosition(h_rect_prof);
...
Y = [pos_ROI_table+((n_px-1)/2) pos_ROI_table+((n_px-1)/2) pos_ROI_table-((n_px-1)/2) pos_ROI_table-((n_px-1)/2)];
...
if isfield(profileAnalysis,'selectedROIs')
    ...
    if any(strcmp(T_old.Properties.VariableNames,'wholeProfileFit'))
        T_new = table(pos_ROI_table,{croppedDataProfile},{croppedDataProfileR},{croppedDataWholeAreaRoi},{croppedDataWholeAreaRoiRaw},{startOfSpark},{endOfSpark},...
            {statEventsSpRec},{detectedEventsMask},h_patch,{posOfEvents},{prevFitCoef},smooth_span,bs_crit,{nan},...
            'VariableNames',{'positionOfRoi' 'dataProfile' 'dataProfileRaw' 'dataWholeArea' 'dataWholeAreaRaw' 'startPosOfSpark' 'endPosOfSpark' ...
            'regionsProp' 'detectedEventsMask' 'patch' 'eventsPeaks' 'prevFitCoef' 'smooth_span' 'bs_crit' 'wholeProfileFit'});
    else
        T_new = table(pos_ROI_table,{croppedDataProfile},{croppedDataProfileR},{croppedDataWholeAreaRoi},{croppedDataWholeAreaRoiRaw},{startOfSpark},{endOfSpark},...
            {statEventsSpRec},{detectedEventsMask},h_patch,{posOfEvents},{prevFitCoef},smooth_span,bs_crit,...
            'VariableNames',{'positionOfRoi' 'dataProfile' 'dataProfileRaw' 'dataWholeArea' 'dataWholeAreaRaw' 'startPosOfSpark' 'endPosOfSpark' ...
            'regionsProp' 'detectedEventsMask' 'patch' 'eventsPeaks' 'prevFitCoef' 'smooth_span' 'bs_crit'});
    ...
    selectedROIs = table(pos_ROI_table,{croppedDataProfile},{croppedDataProfileR},{croppedDataWholeAreaRoi},{croppedDataWholeAreaRoiRaw},{startOfSpark},{endOfSpark},...
                         {statEventsSpRec},{detectedEventsMask},h_patch,{posOfEvents},{prevFitCoef},smooth_span,bs_crit,...
            'VariableNames',{'positionOfRoi' 'dataProfile' 'dataProfileRaw' 'dataWholeArea' 'dataWholeAreaRaw' 'startPosOfSpark' 'endPosOfSpark' ...
                             'regionsProp' 'detectedEventsMask' 'patch' 'eventsPeaks' 'prevFitCoef' 'smooth_span' 'bs_crit'});
    
    selectedROIs.Properties.VariableUnits = {'um' '' '' '' '' 'pixels' 'pixels' '' '' '' '[F/F0, ms]' '' 'ms' ''};
