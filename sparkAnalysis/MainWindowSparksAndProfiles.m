...
mainFig = figure('Name','sparks analysis','units','normalized','outerposition',[0 0.05 1 0.95]);
set(mainFig, 'PaperPositionMode','auto','PaperOrientation',...
              'landscape','PaperType','A4','Tag','sparkAnalysis');

% create axes
ax_img = axes('Parent',mainFig);
set(ax_img,'Position',[0.03 0.72 0.94 0.25])
...
set(ax_img_sparks,'XTick',[],'YDir','reverse','FontSize',14)
set(get(ax_img_sparks,'Ylabel'),'String','x (pixels)','FontWeight','bold')

% set up scale
ax_sc = axes('Parent',mainFig); % invisible axes
set(ax_sc, 'Visible', 'off','Position',[0.975 0.5225 0.005 0.125]);
...
% ax_img_sparks_2 = axes('Parent',mainFig);
% set(ax_img_sparks_2,'Position',get(ax_img_sparks,'Position'))
% % set(get(ax_img_sparks,'Xlabel'),'String','t (ms)')
% set(ax_img_sparks_2,'XTick',[],'YDir','reverse','YAxisLocation','right','FontSize',14)
% set(get(ax_img_sparks_2,'Ylabel'),'String','x (\mum)','FontWeight','bold')

%uistack(ax_img_sparks,'top')

ax_prof = axes('Parent',mainFig,'units','normalized','PickableParts','all');
set(ax_prof,'Position',[0.03 0.26 0.94 0.19],'YGrid','on','FontSize',14)
...
%hObjs.ax_img_sparks_2 = ax_img_sparks_2;
...
h_img_sld = uicontrol('Style', 'slider','Parent',mainFig,'FontUnits','normalized',...
                ...
                'Position', [0.03 0.71 0.94 0.01],'BackgroundColor',[0.7 0.7 0.7],...
                'Callback', '','Enable','off'); %{@imgShow_slider}

h_partAnalysis_pb = uicontrol('Style', 'pushbutton',...
    'String','<html> <p align="center"> image <br> parts <br> analysis <html>',...
    'FontUnits','normalized','FontSize',0.2,...
    'Parent',mainFig,'Units','normalized','Position', [0.97 0.93 0.03 0.07],...
    'Callback',{@analyzePartsOfImage,mainFig},'Enable','on');

% add checkbox and editbox to estimate position of UVflash from image
h_check_UVflash = uicontrol('Style', 'checkbox', 'Parent',mainFig,...
    'FontUnits','normalized', 'Value',0, 'Units','normalized', ...
    'FontSize',0.2, ...
    'String','<html> <p align="center"> get <br> UVflash <br> position <html>', ...
    'Position',[0.97 0.85 0.03 0.05],...
    'Callback',{@getUVflashPos,mainFig});

h_edit_UVflash = uicontrol('Style','edit','String','nan',...
    'FontUnits','normalized','FontSize',0.5,...
    'Parent',mainFig,'Units','normalized',...
    'Position',[0.971 0.825 0.029 0.025],...
    'Callback', {@getUVflashPos,mainFig});
uicontrol('Style','text','String','(ms)',...
    'FontUnits','normalized','FontSize',0.5,...
    'Parent',mainFig,'Units','normalized',...
    'Position',[0.971 0.805 0.029 0.02]);
...
hObjs.h_partAnalysis_pb = h_partAnalysis_pb;
hObjs.h_check_UVflash = h_check_UVflash;
hObjs.h_edit_UVflash = h_edit_UVflash;
...
h_pb_open = uicontrol('Style', 'pushbutton',...
    ...
    'Parent',mainFig,'Units','normalized','Position', [0.01 0.12 0.05 0.1],...
...
h_pb_crop = uicontrol('Style', 'pushbutton',...
    ...
    'FontUnits','normalized','FontSize',0.2,...
    'Parent',mainFig,'Units','normalized','Position', [0.01 0.01 0.05 0.1],...
...
h_table_notes = uitable('Data',{'ctrl';'50 uM BIST-2EGTA';'50 uM Rhod2';'100 uM EGTA';'60 nM free Ca';'P (after shutter) = 60 mW'},...
    'ColumnEditable',true,...
...
%% panel for blank
hp3 = uipanel('Title','blank','Parent',mainFig,'Position',[0.17 0.12 0.09 0.1],...
    'Visible', 'on','Units','normalized','FontUnits','normalized',...
    'FontSize',0.21,'FontWeight','bold');

h_push_BlankROI = uicontrol('Style', 'pushbutton','String','set ROI for blank',...
    'FontUnits','normalized','Parent',hp3,'Units','normalized','FontSize',0.5,...
    'Position', [0.1 0.5 0.8 0.45],'Callback', {@setBlankROI,mainFig});
...
h_pb_GetBlank = uicontrol('Style', 'pushbutton',...
    ...
    'Parent',hp3,'Units','normalized','Position', [0.05 0.05 0.5 0.45],...
...
%% panel normalization 
hp5 = uipanel('Title','self-ratio','Parent',mainFig,'Position',[0.17 0.01 0.09 0.1],...
...
h_pb_norm = uicontrol('Style', 'pushbutton','FontUnits','normalized',...
    ...
    'Parent',hp5,'Units','normalized','Position', [0.05 0.52 0.9 0.45],...
...
h_pb_undo_norm = uicontrol('Style', 'pushbutton','String','start over',...
    'FontUnits','normalized','FontSize',0.5,...
    'Parent',hp5,'Units','normalized','Position', [0.05 0.03 0.9 0.45],...
...
txt1 = uicontrol('Style','text','Parent',hp_crit,'Units','normalized',...
        'Position', [0.05 0.85 0.55 0.125],'String','FWHM (px)',...
...
h_edit_fFWHM = uicontrol('Style','edit','String','8',...
        'Parent',hp_crit,'Units','normalized','Position', [0.65 0.825 0.3 0.15],...
        'TooltipString',['<html> The full-width-at-half-maximum (FWHM) of the Gaussian spatial filter. <br>',... 
        'This should be <= the FWHM of a typical spark, defined in terms of pixels. <br> (Default = 8) <html>'],...
...
h_edit_smoothIter = uicontrol('Style', 'edit','String','3',...
       'Parent',hp_crit,'Units','normalized','Position', [0.65 0.625 0.3 0.15],...
        'TooltipString',['<html> The number of smoothing iterations to apply along the temporal dimension. <br>',... 
        'Higher values suppress noise, but excessively high values cause sparks to be suppressed too. <br> (Default = 3) <html>'],...
        'Callback', {@setCriteriaSparkDetection,mainFig});
...
h_edit_baseIter = uicontrol('Style','edit','String','5',...
        'Parent',hp_crit,'Units','normalized','Position', [0.65 0.425 0.3 0.15],...
        'TooltipString',['<html> The number of smoothing iterations to apply for estimating the baseline. <br>',... 
             'It must be greater than SMOOTH_ITER. <br>',...
             'If sparks occur from a constant baseline, high values can be used <br>',...
             '(effectively resulting in the mean fluorescence of the row being used to estimate the baseline). <br>',...
...
h_edit_tresh = uicontrol('Style', 'edit','String','5',...
        'Parent',hp_crit,'Units','normalized','Position', [0.65 0.225 0.3 0.15],...
        'TooltipString','Threshold value, defined in terms of noise standard deviations. (Default = 5)',...
        'Callback', {@setCriteriaSparkDetection,mainFig});
...
h_edit_expFactor = uicontrol('Style', 'edit','String','0.5',...
        'Parent',hp_crit,'Units','normalized','Position', [0.65 0.025 0.3 0.15],...
        'TooltipString',['<html> EXPANSION_FACTOR - A value between 0 and 1 that determines the size of detected spark regions. <br>',... 
                         'Lower values give larger regions. A value of 1 means that sparks are detected by simply thresholding the processed image at treshold. <br>',... 
                         'Such regions might be too small to contain the peaks of events. (Default = 0.5) <html>'],...
                         'Callback', {@setCriteriaSparkDetection,mainFig});
...
editArray = {h_edit_fFWHM,h_edit_smoothIter,h_edit_baseIter,h_edit_tresh,h_edit_expFactor};
txtArray = {txt1,txt2,txt3,txt4,txt5};
cellfun(@(x) set(x,'FontUnits','normalized','FontSize',0.75,'FontWeight','normal'),txtArray)
cellfun(@(x) set(x,'FontUnits','normalized','FontSize',0.75,'FontWeight','normal'),editArray)
...
h_push_DetectSparks = uicontrol('Style', 'pushbutton',...
    ...
    'FontUnits','normalized','FontSize',0.25,'FontWeight','bold','Parent',hp_SD,...
    'Units','normalized','Position', [0.325 0.7 0.4 0.3],...
    'Callback',{@eventsDetection,mainFig},'Enable','on','Tag','findEvents');
  
txt_SpF = uicontrol('Style','text','FontUnits','normalized',...
    'Parent',hp_SD,'Units','normalized','FontSize',0.7,...
    'Position', [0.325 0.55 0.4 0.125],'String',['# sp*100',char(956),'m-1*s-1'],...
    'ForegroundColor','r');

check_visRect = uicontrol('Style', 'checkbox','Parent',hp_SD,...
    'FontUnits','normalized','Value',1,'Units','normalized','FontSize',0.75,...
    'String','show detected sparks rect','Position', [0.325 0.4 0.4 0.1],...
    'Callback',{@checkRectVisibility,mainFig});

check_watershed = uicontrol('Style', 'checkbox','Parent',hp_SD,...
    'FontUnits','normalized','Value',1,'Units','normalized','FontSize',0.75,...
    'String','watershed transform','Position', [0.325 0.275 0.4 0.1],...
...
txt_spDet = uicontrol('Style','text','FontUnits','normalized',...
    'Parent',hp_SD,'Units','normalized','FontSize',0.7,...
    'Position', [0.325 0.175 0.4 0.1],'String','watershed sensitivity 50%','Enable','on');

sld_spDet = uicontrol('Style', 'slider','Parent',hp_SD,'FontUnits','normalized',...
    'Min',1,'Max',100,'Value',50,'Units','normalized',...
    'Position', [0.325 0.05 0.4 0.1],...
    'Callback', {@spDet_slider,mainFig},'Enable','on');

uicontrol('Style','text',...
        'Parent',hp_SD,'Units','normalized',...
        'Position', [0.75 0.8 0.225 0.2],'String','event ROI duration (ms)',...
...
uicontrol('Style','text',...
        ...
        'FontUnits','normalized','FontSize',0.75);
h_edit_MinDurSpark = uicontrol('Style', 'edit','String','20',...
        'Parent',hp_SD,'Units','normalized','Position', [0.75 0.55 0.1 0.15],...
        'FontUnits','normalized','FontSize',0.5,...
        'Callback', {@setCriteriaSparkDetection,mainFig});
h_edit_MaxDurSpark = uicontrol('Style', 'edit','String','inf',...
        'Parent',hp_SD,'Units','normalized','Position', [0.875 0.55 0.1 0.15],...
        'FontUnits','normalized','FontSize',0.5,...
        'Callback', {@setCriteriaSparkDetection,mainFig});
...
uicontrol('Style','text',...
        ...
        'FontUnits','normalized','FontSize',0.75);
h_edit_MinWidthSpark = uicontrol('Style', 'edit','String','1',...
        'Parent',hp_SD,'Units','normalized','Position', [0.75 0.05 0.1 0.15],...
        'FontUnits','normalized','FontSize',0.5,...
        'Callback', {@setCriteriaSparkDetection,mainFig});
h_edit_MaxWidthSpark = uicontrol('Style', 'edit','String','inf',...
        'Parent',hp_SD,'Units','normalized','Position', [0.875 0.05 0.1 0.15],...
        'FontUnits','normalized','FontSize',0.5,...
        'Callback', {@setCriteriaSparkDetection,mainFig});

% % slider for spark detection sensitivity
% txt_spDet = uicontrol('Style','text','FontUnits','normalized',...
%     'Parent',hp_SD,'Units','normalized','FontSize',0.7,...
%     'Position', [0.52 0.1 0.46 0.1],'String','spDet sensitivity 50%','Enable','on');
% 
% sld_spDet = uicontrol('Style', 'slider','Parent',hp_SD,'FontUnits','normalized',...
%     'Min',1,'Max',99,'Value',50,'Units','normalized',...
%     'Position', [0.52 0.01 0.46 0.1],...
%     'Callback', {@spDet_slider,mainFig},'Enable','on');
...
switch analysisType
    ...
    case 'spark detection'
        ...
        %%
        hp_SA = uipanel('Title','analyze detected sparks','Parent',mainFig,...
            'Units','normalized','Position',[0.515 0.01 0.3 0.21],...
            'FontUnits','normalized','FontSize',0.1,'FontWeight','bold');
        
        % criteria
        hp_cSR = uipanel('Title','selection criteria:','Parent',hp_SA,'Units','normalized',...
            'Position',[0.01 0.01 0.325 0.95],'FontUnits','normalized',...
            'FontSize',0.1,'FontAngle','italic');
        
        txt_noise = uicontrol('Style','text',...
            'Parent',hp_cSR,'Units','normalized','Position', [0 0.8 0.6 0.2],...
            'String',{'spAmpl >';'3*noise (F/F0)'},...
        ...
        h_edit_noise = uicontrol('Style', 'edit','String','0.2',...
            ...
            'Position', [0.6 0.8 0.35 0.175],'Callback',{@sparkParamsFiltering,mainFig},...
            'TooltipString','amplitude of spark is higher than noise estimated as SD of normalizde image without sparks area');
        ...
        h_edit_spFDHM = uicontrol('Style', 'edit','String','5',...
            'Parent',hp_cSR,'Units','normalized','Position', [0.6 0.55 0.35 0.175],...
            'TooltipString','FDHM of spark is higher than set value',...
            'Callback',{@sparkParamsFiltering,mainFig});
        
        txt_spFWHM = uicontrol('Style','text',...
            'Parent',hp_cSR,'Units','normalized',...
            'Position', [0 0.3 0.6 0.2],'String',{'spFWHM >';['(',char(956),'m)']},...
            'TooltipString','FWHM of spark is higher than set value');
        
        h_edit_spFWHM = uicontrol('Style', 'edit','String','0.25',...
            'Parent',hp_cSR,'Units','normalized','Position', [0.6 0.3 0.35 0.175],...
            'TooltipString','FWHM of spark is higher than set value',...
            'Callback',{@sparkParamsFiltering,mainFig});
        
        h_smooth_txt = uicontrol('Style','text',...
            'Parent',hp_cSR,'Units','normalized',...
            'Position', [0 0.05 0.6 0.2],'String',{'span of smoothing:';'(ms)\loess'});
        
        h_smooth_edit = uicontrol('Style','edit','String','50',...
            'Parent',hp_cSR,'Units','normalized','Position', [0.6 0.05 0.35 0.175]);
        
        editArray2 = {h_smooth_edit,h_edit_spFDHM,h_edit_spFWHM,h_edit_noise};
        txtArray2 = {txt_noise,txt_spFDHM,txt_spFWHM,h_smooth_txt};
        cellfun(@(x) set(x,'FontUnits','normalized','FontSize',0.375,'FontWeight','normal'),txtArray2)
        cellfun(@(x) set(x,'FontUnits','normalized','FontSize',0.65,'FontWeight','normal'),editArray2)
        % save handles
        ...
        calcSpParamMethodRBgroup = uibuttongroup(hp_SA,'Units','normalized',...
            'Title','spark params calc method:','FontUnits','normalized',...
            'FontSize',0.186,'FontWeight','normal','FontAngle','italic',...
            'Position', [0.35 0.45 0.375 0.51]);
      
        rb1 = uicontrol('Style','radiobutton','Parent',calcSpParamMethodRBgroup,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.7,...
            'String','max crossing profiles','Position',[0.05 0.7 0.9 0.3]);
        
        rb2 = uicontrol('Style','radiobutton','Parent',calcSpParamMethodRBgroup,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.7,...
            'String','2D gaussian fit','Position',[0.05 0.35 0.9 0.3],'Enable','on');
        
        rb3 = uicontrol('Style','radiobutton','Parent',calcSpParamMethodRBgroup,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.7,...
            'String','estimate from event img','Position',[0.05 0.01 0.9 0.3],'Enable','on');
        
        check_showEventsFigs = uicontrol('Style', 'checkbox','Parent',hp_SA,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.8,...
            'String','show individual events','Position', [0.35 0.3 0.3 0.1],...
            'Callback',{@analysisOpt,mainFig});
        
        h_bsDet_txt = uicontrol('Style','text','FontUnits','normalized',...
        ...
        h_bsDet_edit = uicontrol('Style','edit','String','75',...
            ...
            'Parent',hp_SA,'Units','normalized','Position', [0.575 0.05 0.1 0.17],'Callback',{@profileROI,mainFig},...
            'TooltipString','sensitivity for baseline estimation when fitting spark rise');
        ...
        h_pb_sparksAnalysis = uicontrol('Style', 'pushbutton',...
            ...
            'FontUnits','normalized','FontSize',0.3,'Parent',hp_SA,'FontWeight','bold',...
            'Units','normalized','Position', [0.75 0.5 0.225 0.35],...
            'Callback',{@detectedSparksAnalysis,mainFig},'Enable','on','Tag','calcParamsOfEvents');
        
        uicontrol('Style','text','FontUnits','normalized',...
            'Parent',hp_SA,'Units','normalized','FontSize',0.55,...
            'Position', [0.725 0.3 0.275 0.15],'String','corrected sparkFreq:',...
            'TooltipString','corrected spark frequency based on selection criteria for sparks',...
            'ForegroundColor','k');
        
        txt_correctedSpF = uicontrol('Style','text','FontUnits','normalized',...
            'Parent',hp_SA,'Units','normalized','FontSize',0.275,...
            'Position', [0.725 0 0.275 0.3],'String',{'#';['sp*100',char(956),'m-1*s-1']},...
            'ForegroundColor','r');
    ...
    otherwise % PROFILES
        ...
        hp_cSR = uipanel('Title','criteria','Parent',hp_SR,'Units','normalized',...
            'Position',[0.01 0.01 0.4 0.95],'FontUnits','normalized',...
            'FontSize',0.1,'FontAngle','italic');
        
        txt_l = uicontrol('Style','text',...
            'Parent',hp_cSR,'Units','normalized','Position', [0.01 0.8 0.64 0.2],...
            'String',{'width of ROI:';'(pixels, odd #)'});
        
        h_edit_ROI = uicontrol('Style', 'edit','String','11',...
            'Parent',hp_cSR,'Units','normalized',...
            'Position', [0.7 0.8 0.25 0.175],'Callback',{@profileROI,mainFig});
        
        txt_averageWidth = uicontrol('Style','text',...
            'Parent',hp_cSR,'Units','normalized',...
            'Position', [0.01 0.55 0.64 0.2],'String',{'diameter (um)','around spark centre'},...
            'TooltipString','calculate profile only from pixels within diameter around spark centre');
        
        h_edit_averageWidth = uicontrol('Style', 'edit','String','1',...
            'Parent',hp_cSR,'Units','normalized','Position', [0.7 0.55 0.25 0.175],...
            'TooltipString','calculate profile only from pixels within diameter around spark centre',...
            'Callback',{@profileROI,mainFig});
        
        txt_thr_l = uicontrol('Style','text',...
            'Parent',hp_cSR,'Units','normalized',...
            'Position', [0.01 0.3 0.64 0.2],'String',{'threshold:';'(#xSD)'});
        
        h_edit_tresh_prof = uicontrol('Style', 'edit','String','5',...
            'Parent',hp_cSR,'Units','normalized','Position', [0.7 0.3 0.25 0.175]);
        
        h_smooth_txt = uicontrol('Style','text',...
            'Parent',hp_cSR,'Units','normalized',...
            'Position', [0.01 0.05 0.64 0.2],'String',{'span of smoothing:';'(ms)\loess'});
        
        h_smooth_edit = uicontrol('Style','edit','String','50',...
            'Parent',hp_cSR,'Units','normalized','Position', [0.7 0.05 0.25 0.175]);
        
        editArray2 = {h_smooth_edit,h_edit_tresh_prof,h_edit_averageWidth,h_edit_ROI};
        txtArray2 = {txt_l,txt_averageWidth,txt_thr_l,h_smooth_txt};
        cellfun(@(x) set(x,'FontUnits','normalized','FontSize',0.375,'FontWeight','normal'),txtArray2)
        cellfun(@(x) set(x,'FontUnits','normalized','FontSize',0.65,'FontWeight','normal'),editArray2)
        
        % table of selected profiles
        h_table_profs = uitable('Data',zeros(5,2),'ColumnEditable',[true false],...
            'ColumnName',{'<HTML> <font size="4"> <b> pos (px) </b> </HTML>', '<HTML> <font size="4"> <b> # events </b> </HTML>'},...
            'ColumnWidth','auto','Parent',hp_SR,...
            'Units','normalized','Position',[0.425 0.325 0.365 0.575],...
        ...
        h_bsDet_txt = uicontrol('Style','text','FontUnits','normalized',...
            ...
            'Position', [0.425 0.01 0.25 0.3],'String',{'baseline detection';'sensitivity';'0-max, 100-min'});
        
        h_bsDet_edit = uicontrol('Style','edit','String','75',...
            'FontUnits','normalized','FontSize',0.55,...
            'Parent',hp_SR,'Units','normalized','Position', [0.69 0.10 0.1 0.17],'Callback',{@profileROI,mainFig});
        
        h_pb_profROI = uicontrol('Style', 'pushbutton',...
            'String','<html> <p align="center"> draw <br> ROI <html>','FontUnits','normalized','FontSize',0.3,...
            'Parent',hp_SR,'Units','normalized','Position', [0.8 0.55 0.19 0.35],...
            'Callback',{@profileROI,mainFig});
        
        h_pb_getPosOfProf = uicontrol('Style', 'pushbutton',...
            'String','<html> <p align="center"> get <br> position <html>','FontUnits','normalized','FontSize',0.3,...
            'Parent',hp_SR,'Units','normalized','Position', [0.8 0.1 0.19 0.35],...
        ...
        hp_Analysis = uipanel('Title','analysis','Parent',mainFig,...
            'Units','normalized','Position',[0.73 0.01 0.155 0.21],...
            'FontUnits','normalized','FontSize',0.1,'FontWeight','bold');
        
        check_showEventsFigs = uicontrol('Style', 'checkbox','Parent',hp_Analysis,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.8,...
            'String','show individual events','Position', [0.01 0.84 0.64 0.1],...
            'Callback',{@analysisOpt,mainFig});
        
        check_sparkParams = uicontrol('Style', 'checkbox','Parent',hp_Analysis,...
            'FontUnits','normalized','Value',1,'Units','normalized','FontSize',0.8,...
            'String','get sparks params','Position', [0.01 0.68 0.64 0.1],...
            'Callback',{@analysisOpt,mainFig});
        
        % method for spark parameters calculation, create two radio buttons in the button group
        calcSpParamMethodRBgroup = uibuttongroup(hp_Analysis,'Units','normalized',...
            'Title','spark params calc method','FontUnits','normalized',...
            'FontSize',0.2,'FontWeight','bold','FontAngle','normal',...
            'Position', [0.01 0.22 0.64 0.4]);
        
        rb1 = uicontrol('Style','radiobutton','Parent',calcSpParamMethodRBgroup,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.7,...
            'String','max crossing profiles','Position',[0.05 0.5 0.9 0.4]);
        
        rb2 = uicontrol('Style','radiobutton','Parent',calcSpParamMethodRBgroup,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.7,...
            'String','2D gaussian fit','Position',[0.05 0.05 0.9 0.4],'Enable','on');
        
        check_pairAnalysis = uicontrol('Style', 'checkbox','Parent',hp_Analysis,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.8,...
            'String','pairwise analysis','Position', [0.01 0.06 0.64 0.1],...
            'Callback','');
        
        h_pb_sparksAnalysis = uicontrol('Style', 'pushbutton',...
            'String','<html> <p align="center"> analyze <br> sparks <html>',...
            'FontUnits','normalized','FontSize',0.35,'Parent',hp_Analysis,'FontWeight','bold',...
        ...
        h_pb_profilesFit = uicontrol('Style', 'pushbutton',...
            ...
            'Parent',hp_Analysis,'Units','normalized','Position', [0.675 0.35 0.3 0.3],...
        ...
        h_pb_profileAnalysis = uicontrol('Style', 'pushbutton',...
            ...
            'Parent',hp_Analysis,'Units','normalized','Position', [0.675 0.025 0.3 0.3],...
        ...
        hObjs.check_pairAnalysis = check_pairAnalysis;
...
switch analysisType
    ...
    case 'spark detection'
        ...
        hp_Analysis = uipanel('Title','save','Parent',mainFig,...
            'Units','normalized','Position',[0.82 0.01 0.1 0.21],...
            'FontUnits','normalized','FontSize',0.1,'FontWeight','bold');
        ...
        h_pb_batchAnalysis = uicontrol('Style', 'pushbutton',...
            ...
            'FontUnits','normalized','FontSize',0.275,'FontWeight','bold',...
            'Parent',hp_Analysis,'Units','normalized','Position', [0.05 0.05 0.9 0.4],...
        ...
        h_pb_reAnalyze = uicontrol('Style', 'pushbutton',...
            ...
            'Parent',mainFig,'Units','normalized','Position', [0.9275 0.15 0.065 0.06],...
            'Callback',{@reAnalyze,mainFig},'Enable','on');
        
        hObjs.h_pb_reAnalyze = h_pb_reAnalyze;
        

        %% create COCO-like dataset for machine learning problem
        hp_datasetForAIdetection = uipanel('Title','create COCO-like dataset','Parent',mainFig,...
            'Units','normalized','Position',[0.925 0.01 0.07 0.14],...
            'FontUnits','normalized','FontSize',0.065,'FontWeight','bold');

        h_pb_saveImgForML = uicontrol('Style', 'pushbutton',...
            'String','<html> <p align="center"> save img <br/> <i> <span style="font-size:100%">and mask for ML</span></i> <html>',...
            'FontUnits','normalized','FontSize',0.275,'FontWeight','bold',...
            'Parent',hp_datasetForAIdetection,'Units','normalized',...
            'Position', [0.025 0.55 0.95 0.4],...
            'Callback',{@saveImgForCOCOlikeDataset,mainFig},'Enable','on');
        
        h_pb_createFinalDataset = uicontrol('Style', 'pushbutton',...
            'String','<html> <p align="center"> create final datasets <br/> <i> <span style="font-size:75%">(train, val, test)</span></i> <html>',...
            'FontUnits','normalized','FontSize',0.225,'FontWeight','bold',...
            'Parent',hp_datasetForAIdetection,'Units','normalized',...
            'Position', [0.025 0.025 0.65 0.5],...
            'Callback',{@createFinalCOCOlikeDataset,mainFig},'Enable','on');
        
        uicontrol('Style','text',...
            'String',{'image','size (px)'},...
            'FontUnits','normalized','FontSize',0.35,...
            'Parent',hp_datasetForAIdetection,'Units','normalized',...
            'Position', [0.65 0.35 0.35 0.225],'Callback','');
        
        h_maxImgDimSz_edit = uicontrol('Style','edit','String','256',...
            'FontUnits','normalized','FontSize',0.35,...
            'Parent',hp_datasetForAIdetection,'Units','normalized',...
            'Position', [0.675 0.025 0.3 0.3],'Callback','');
        
        hObjs.h_maxImgDimSz_edit = h_maxImgDimSz_edit;
        hObjs.h_pb_saveImgForML = h_pb_saveImgForML;
        hObjs.h_pb_createFinalDataset = h_pb_createFinalDataset;
        
        
    otherwise
        
        hp_Analysis = uipanel('Title','save','Parent',mainFig,...
            'Units','normalized','Position',[0.89 0.115 0.1 0.105],...
            'FontUnits','normalized','FontSize',0.2,'FontWeight','bold');
        
        h_pb_saveAnalysis = uicontrol('Style', 'pushbutton',...
            'String','save analysis',...
            'FontUnits','normalized','FontSize',0.55,'FontWeight','bold',...
            'Parent',hp_Analysis,'Units','normalized','Position', [0.1 0.525 0.8 0.475],...
            'Callback',{@saveSparkRecAnalysis,mainFig});
        
        check_saveEventsFigs = uicontrol('Style', 'checkbox','Parent',hp_Analysis,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.9,...
            'String','save individual events','Position', [0.01 0.3 0.95 0.2]);
        
        check_saveProfsAndFits = uicontrol('Style', 'checkbox','Parent',hp_Analysis,...
            'FontUnits','normalized','Value',0,'Units','normalized','FontSize',0.9,...
            'String','save profs & fits','Position', [0.01 0.01 0.95 0.2]);
        ...
        h_pb_reAnalyze = uicontrol('Style', 'pushbutton',...
            ...
            'FontUnits','normalized','FontSize',0.2,'FontWeight','bold',...
            'Parent',mainFig,'Units','normalized','Position', [0.89 0.01 0.045 0.09],...
            'Callback',{@reAnalyze,mainFig},'Enable','on');
        ...
        h_pb_dataPreview = uicontrol('Style', 'pushbutton',...
            ...
            'Parent',mainFig,'Units','normalized','Position', [0.94 0.01 0.045 0.09],...
            'Callback',{@experimentPreviewSparkRecovery,mainFig},'Enable','on');
