function params = calcParametersOfEventsFromWholeProfileFit...
    (t,splFit,t_ups,splFit_ups,coefFitEventRise,startOfEvent,Npeaks,...
    pks,locs,expFitS,expFitE)
...
for i = 1:Npeaks
    ...
    eventFit_ups = [ ones(eS-1,1).*bs(i); splFit_ups(eS:eE); ones(numel(t_ups)-eE,1).*bs(i) ];
    eventFit = [ ones(eSr-1,1).*bs(i); splFit(eSr:eEr); ones(numel(t)-eEr,1).*bs(i) ];
...
% plot( abs(abs(y_t1 - half_max)-max(abs(y_t1 - half_max))) )
    ...
    try
        ...
        opt = optimoptions('lsqnonlin','MaxIterations',1000,'MaxFunctionEvaluations',3000);
        coefExp = lsqnonlin(@(p)funExpFit(p,t_exp(posExp:end),y_exp(posExp:end)),...
            [vMr,t_exp(posExp),20,bs(i)],[0 0 0 -inf],[inf inf inf inf],opt);
...
%         plot(t_exp,funExpEval(coefExp,t_exp),'r')
    ...
    try
        eventsParams(i,:) = [t0(i), bs(i), t_ups(pM), half_max_1, half_max_2,...
            half_max, vM, amplitude, TTP, FDHM, AUC, tauD, firstDerVal, t(firstDerPos)];
    catch
        eventsParams(i,:) = [nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan];
    end
...
% save data 
names_params = {'t0 (ms)','F0 (deltaF/F0)','tPeak (ms)','halfMax1 (ms)','halfMax2 (ms)','halfMax (deltaF/F0)',...
    'Apeak (deltaF/F0)','amplitude (deltaF/F0)','TTP (ms)','FDHM (ms)','AUC (ms*deltaF/F0)',...
    'tauD (ms)','firstDerMaxVal (deltaF/F0)','firstDerMaxValPos (ms)','expDecayFit'};
