function out = multipleEventsFitFcnNormBs(pos_peak,t,y,baselineModel,...
            fitFun,t_ups,recalculate,mEvents,mBaseline,ax,...
            useWeights,sPP,prevFitCoefSpRise)
...
% useWeights
% sPP = starting positions of photolysis
...
[fun_event,p0,lb,ub,n_coef_event,coeff_n] = setFitFunction(...
    fitFun,N_events,t,y,val_peak,pos_peak,sPP,prevFitCoefSpRise);

% do fitting
events_coeff = fitOfData(...
    p0,t,y,n_coef_event,fun_event,N_events,fit_baseline,mEvents,fitFun,ax,useWeights,lb,ub);
...
if exist('t_ups', 'var')
    ...
    [F_ups,F_events_ups,F_individualEvents_ups] = sumEventsOutput(events_coeff,t_ups,n_coef_event,fun_event,N_events,feval(f_baseline,t_ups));
...
% % %             
%             [F,F_events,F_individualEvents] = sumEvents(p0(p0_mask),tp,n_coef_event,fun_event,N_events_part,bp);
% % %             
% % %             
%              figure
%              plot(tp,yp)
...
function [fun_event,p0,lb,ub,n_coef_event,coeff_n] = ...
    setFitFunction(fitFun,N_events,t,y,val_peak,pos_peak,sPP,prevFitCoefSpRise)
...
%                            (t>p(2)).*( p(4).*(1-exp(-(p(2)-p(1))./p(3))).*exp(-(t-p(2))./p(5)) );
    ...
    case 'CaSpikeFun'
        ...
        for i=1:N_events
            ...
            if sPP(i) > 2
                lb(1 + (i-1)*n_coef_event) = t(sPP(i)-1);  
            end
