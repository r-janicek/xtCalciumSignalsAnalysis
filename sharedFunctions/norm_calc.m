...
if hObjs.check_doBsFit.Value
    ...
    typeFit = hObjs.popUpMenuBaselineFcn.String{hObjs.popUpMenuBaselineFcn.Value};
    ...
    for i = 1:size(crop_F0,1)
        ...
        waitbar(i/size(crop_F0,1),hw,sprintf('%d %%',round((i/size(crop_F0,1))*100)))
        
        F0(i,:) = fitBaseline(t,imgDataXTfluoFN(i,:),typeFit,mBs,0,[]);
        F0raw(i,:) = fitBaseline(t,imgDataXTfluoRN(i,:),typeFit,mBs,0,[]);
        
        img_data_n(i,:) = (imgDataXTfluoFN(i,:)-F0(i,:)) ./ (F0(i,:) - blank);
...
set(get(ax_prof,'Ylabel'),'String','fluorescence (deltaF/F0)','FontWeight','bold')
